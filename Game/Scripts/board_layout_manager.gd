...
static func draw_board_ninepatch_frame(
	canvas: CanvasItem,
	tex: Texture2D,
	grid_rect: Rect2,
	margins: Vector4i,
	...
	if canvas == null or tex == null or margins == Vector4i.ZERO:
		return
	var sz := tex.get_size()
	...
	var tex_cell_w := inner_w / float(gw)
	var tex_cell_h := inner_h / float(gh)

	# Corners
	if not ("C_TL" in hidden_pieces):
		canvas.draw_texture_rect_region(tex, Rect2(grid_rect.position.x - f_left, grid_rect.position.y - f_top, f_left, f_top), Rect2(0, 0, L, T), modulate)
	if not ("C_TR" in hidden_pieces):
		canvas.draw_texture_rect_region(tex, Rect2(grid_rect.position.x + grid_rect.size.x, grid_rect.position.y - f_top, f_right, f_top), Rect2(L + inner_w, 0, R, T), modulate)
	if not ("C_BL" in hidden_pieces):
		canvas.draw_texture_rect_region(tex, Rect2(grid_rect.position.x - f_left, grid_rect.position.y + grid_rect.size.y, f_left, f_bottom), Rect2(0, T + inner_h, L, B), modulate)
	if not ("C_BR" in hidden_pieces):
		canvas.draw_texture_rect_region(tex, Rect2(grid_rect.position.x + grid_rect.size.x, grid_rect.position.y + grid_rect.size.y, f_right, f_bottom), Rect2(L + inner_w, T + inner_h, R, B), modulate)

	# Top edge segments (cell by cell)
	for x in range(gw):
		var key := "T_%d" % x
		if not (key in hidden_pieces):
			var dst := Rect2(grid_rect.position.x + float(x) * cell_w, grid_rect.position.y - f_top, cell_w, f_top)
			var src := Rect2(L + float(x) * tex_cell_w, 0, tex_cell_w, T)
			canvas.draw_texture_rect_region(tex, dst, src, modulate)

	# Bottom edge segments (cell by cell)
	for x in range(gw):
		var key := "B_%d" % x
		if not (key in hidden_pieces):
			var dst := Rect2(grid_rect.position.x + float(x) * cell_w, grid_rect.position.y + grid_rect.size.y, cell_w, f_bottom)
			var src := Rect2(L + float(x) * tex_cell_w, T + inner_h, tex_cell_w, B)
			canvas.draw_texture_rect_region(tex, dst, src, modulate)

	# Left edge segments (cell by cell)
	for y in range(gh):
		var key := "L_%d" % y
		if not (key in hidden_pieces):
			var dst := Rect2(grid_rect.position.x - f_left, grid_rect.position.y + float(y) * cell_h, f_left, cell_h)
			var src := Rect2(0, T + float(y) * tex_cell_h, L, tex_cell_h)
			canvas.draw_texture_rect_region(tex, dst, src, modulate)

	# Right edge segments (cell by cell)
	for y in range(gh):
		var key := "R_%d" % y
		if not (key in hidden_pieces):
			var dst := Rect2(grid_rect.position.x + grid_rect.size.x, grid_rect.position.y + float(y) * cell_h, f_right, cell_h)
			var src := Rect2(L + inner_w, T + float(y) * tex_cell_h, R, tex_cell_h)
			canvas.draw_texture_rect_region(tex, dst, src, modulate)

static func get_frame_piece_rects(
	grid_rect: Rect2,
	margins: Vector4i,
	tex_size: Vector2,
	grid_w: int,
